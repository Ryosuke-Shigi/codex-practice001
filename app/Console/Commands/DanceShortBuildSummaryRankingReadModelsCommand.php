...
/**
 * DanceShortsRadar のまとめ ranking read model を enabled pattern 単位で同期生成する Command です。
 */
class DanceShortBuildSummaryRankingReadModelsCommand extends Command
{
    protected $signature = 'dance-shorts-radar:build-summary-ranking-read-models';

    protected $description = 'Build DanceShortsRadar summary ranking read model patterns synchronously.';
    ...
    public function handle(
        ...
        try {
            ...
            $this->error('DanceShortsRadar summary ranking read model patterns build failed.');
            $this->line('error: '.$exception->getMessage());

            return self::FAILURE;
        }

        $this->info('DanceShortsRadar summary ranking read model patterns built.');
        $this->line('summary_patterns: '.count($definitions));
