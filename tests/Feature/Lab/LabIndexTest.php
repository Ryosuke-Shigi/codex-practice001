...
class LabIndexTest extends TestCase
{
    ...
    public function test_lab_lists_project_idea_board_and_mock_entries_in_display_order(): void
    {
        ...
        $this
            ...
            ->assertInertia(fn (Assert $page) => $page
                ...
                ->where('experiments.4.id', 'quake-wave-map-idea-board')
                ->where('experiments.4.title', 'Japan Quake Wave Map')
                ->where('experiments.4.status', '完成済み')
                ->where('experiments.4.category', 'IDEA-BOARD')
                ->where('experiments.4.href', '/lab/quake-wave-map-idea-board')
                ...
                ->where('experiments.6.id', 'construction-order-workflow-idea-board')
                ->where('experiments.6.title', '工事発注管理・請求システム')
                ->where('experiments.6.status', 'アイデアボード')
                ->where('experiments.6.category', 'IDEA-BOARD')
                ->where('experiments.6.href', '/lab/construction-order-workflow-idea-board')
                ...
                ->where('experiments.11.id', 'dance-shorts-radar-mock')
                ->where('experiments.11.title', 'Dance Shorts Radar モック')
                ->where('experiments.11.status', 'Mock')
                ->where('experiments.11.category', 'MOCK')
                ->where('experiments.11.href', '/lab/dance-shorts-radar-mock')
            );
    }
    ...
    public function test_lab_keeps_existing_mock_links(): void
    {
        ...
        $this
            ...
            ->assertInertia(fn (Assert $page) => $page
                ->component('Lab/Index', false)
                ->where('experiments.8.id', 'api-preview')
                ->where('experiments.8.title', 'API Preview')
                ->where('experiments.8.status', 'Mock')
                ->where('experiments.8.category', 'MOCK')
                ->where('experiments.8.href', '/api-preview')
                ->where('experiments.9.id', 'quakewave-preview-tools')
                ->where('experiments.9.title', 'QuakeWave Preview')
                ->where('experiments.9.status', 'Mock')
                ->where('experiments.9.category', 'MOCK')
                ->where('experiments.9.href', '/quakewave-preview')
                ->where('experiments.10.id', 'construction-order-workflow-mock')
                ->where('experiments.10.title', '工事発注管理・請求システム モック')
                ->where('experiments.10.status', 'Mock')
                ->where('experiments.10.category', 'MOCK')
                ->where('experiments.10.href', '/lab/construction-order-workflow-mock')
            );
    }
