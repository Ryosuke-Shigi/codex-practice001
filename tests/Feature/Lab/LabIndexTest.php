    ...
    public function test_lab_lists_project_idea_board_and_mock_entries_in_display_order(): void
    {
        ...
        $this
            ...
            ->assertInertia(fn (Assert $page) => $page
                ...
                ->where('experiments.10.id', 'dance-shorts-analyzer-mock')
                ->where('experiments.10.title', 'DanceShortsAnalyzer MOCK')
                ->where('experiments.10.status', 'Mock')
                ->where('experiments.10.category', 'MOCK')
                ->where('experiments.10.href', '/lab/dance-shorts-analyzer-mock')
    ...
    public function test_lab_keeps_existing_mock_links(): void
    {
        ...
        $this
            ...
            ->assertInertia(fn (Assert $page) => $page
                ...
                ->where('experiments.11.category', 'MOCK')
                ->where('experiments.11.href', '/lab/dance-shorts-radar-mock')
                ->where('experiments.12.id', 'quakewave-preview-tools')
                ->where('experiments.12.title', 'QuakeWave Preview')
                ->where('experiments.12.status', 'Mock')
                ->where('experiments.12.category', 'MOCK')
                ->where('experiments.12.href', '/quakewave-preview')
                ->where('experiments.13.id', 'construction-order-workflow-mock')
                ->where('experiments.13.title', '工事発注管理・請求システム モック')
                ->where('experiments.13.status', 'Mock')
                ->where('experiments.13.category', 'MOCK')
                ->where('experiments.13.href', '/lab/construction-order-workflow-mock')
            );
    }
