...
発想から本実装までの段階、各段階の完成条件、次へ進む条件、Product実装の基本手順を定めます。

MOCK / Prototypeの配置、Route、共有範囲、Productとの物理的分離は `docs/prototype-policy.md` を正本とします。
...
前段階のコードをそのまま次段階へ昇格させず、確認できた内容を仕様として抽出して進めます。
...
- 業務判断・権限判断
- 正式な状態遷移
- Product用Action、Service、Repository
...
速度を優先して一気に作ってよいのはPrototypeまでです。速く作れたことを完成の根拠にはしません。
...
具体的な配置と分離境界は `docs/prototype-policy.md`、Commonの責務は `docs/ui.md` に従います。

## テストとレビュー

共通テスト方針は `docs/testing.md` に従います。

重要な境界は実装前または実装と同時に固定します。並び順、pagination props、表示補助データ、モバイル固有操作などは、挙動が明確になった後に固定してよいものとします。
...
- 人間による完成判定・merge・正式反映判断

## 関連文書

- MOCK / Prototypeの配置と分離: `docs/prototype-policy.md`
