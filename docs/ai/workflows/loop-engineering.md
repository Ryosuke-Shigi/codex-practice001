...
親が監査対象を固定し、利用可能なら実装担当と分離した仕様reviewerが成立性を確認します。利用できない場合は親が同じ観点を順番に確認します。仕様の採否は人間が判断し、未成立のまま実装へ進みません。
...
writer修正後に、利用可能なら実装担当と分離したbrowser検証担当が同一条件で再実行し、mobile、tablet、PCと既存導線を回帰確認します。利用できない場合は親が同じ条件を順番に確認します。browserが利用不能な場合は未確認範囲を分け、代替確認を実画面確認済みとしません。

### レビューループ

実装と検証後に、利用可能なら実装担当と分離したreviewerが指示、正本、差分、TDD、Sensors、browser結果を独立確認します。利用できない場合は親が同じ観点を順番に確認し、自己確認だけで十分かをリスクに応じて明示します。
...
軽量な実行担当で解決できなければ専門知識を持つ利用可能な担当へ昇格し、複雑・高リスク・複数レイヤーで適切な担当がなければ人間判断へ返します。
...
- project設定や利用可能な実行設定の値とruntime実測を分離できない。
