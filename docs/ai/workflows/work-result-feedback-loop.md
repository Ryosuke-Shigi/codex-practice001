...
1. Draft PRの本文を書く前に、目的、実際のdiff、変更した成果物と責務、レビュー強度、branch・commit、参照した正本、実行した完全なコマンドと実行root、検証結果、実施した照合、CIの現在状態、Sensorsの判定と根拠、確認済み・未確認・未実行・対象外の範囲を回収し、事実一覧として一度列挙する。Docker Compose経由のコマンドはCompose root、service、実行経路を分け、Codex Browserと人間の確認範囲を混同しない
2. [PR用まとめテンプレート](../../templates/pr-summary.md) に従い、事実一覧の実施済み内容と未確認範囲をPR本文の既存欄へ反映する
3. Draft PR作成後にCI等の新しい結果が得られた場合は、実際の結果で本文を更新する。pendingや未確認を推測でsuccessへ変えない
4. 事実一覧の全項目が本文へ反映され、本文がdiff、コマンド結果、CI、Sensors、確認範囲と一致することを再照合する。「未実施」と「実施済みだが本文未記載」を混同しない
5. 記載漏れまたは誤った断定があれば、完了報告前にPR本文を修正し、同じ観点で再照合する
...
1. 上記の事実回収とPR用まとめテンプレートに従い、実行結果、未実行理由、docs更新要否、該当SensorsをPR Summaryへ戻す
