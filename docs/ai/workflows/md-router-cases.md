...
| React UIだけ修正 | 画面表示、操作、UI状態を修正する | `../../guides/frontend-screen-types.md` / `../../frontend.md` / `../../ui.md` / 対象feature docs | Repository / Migration | Page / Component / hook / type | propsやDB条件の変更が必要になる | `docker compose run --rm npm npm run build` | UI影響範囲、画面種別、確認した画面、build結果 |
