...
## ChatGPT / CodexApp の責務境界

ChatGPT は、仕様整理、責務整理、指示用まとめ作成、圧縮、圧縮後チェック、PRレビュー観点整理を担当する。

CodexApp は、ChatGPTから渡された指示用まとめに従い、対象MD確認、既存コード調査、実装、差分確認、テスト実行を担当する。

CodexApp は、指示用まとめを再設計しない。
CodexApp は、指示用まとめを再圧縮しない。
CodexApp は、読むMD、読まないMD、作るもの、作らないもの、成功条件、失敗条件、テスト観点、確認コマンドを削除・変更しない。

指示用まとめとdocsまたはコードが矛盾する場合は、自己判断で統合せず停止して報告する。
