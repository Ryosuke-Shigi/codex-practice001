...
- 色だけで状態を表現しない
- disabled、loading、error、emptyの状態を画面上で判別できるようにする
- 自動再生、自動送り、アニメーションには停止手段を用意する
- タップ領域が小さくなりすぎないようにする

## データ取得と画面更新

- 初期表示に必要なデータはInertia propsで渡す
- 画面内の軽い状態変更はReact stateで扱う
- 追加データ取得が必要な場合は、取得単位と通信回数を先に決める
- 一度に大量のデータをpropsへ載せない
- 表示中のデータと次に必要なデータを分ける
- モバイル通信量、画像サイズ、再描画回数を確認する
- 同じデータをLaravelとReactの両方で重複計算しない

## Component分割の判断基準

次の場合にComponent分割を検討します。

- 表示責務が複数ある
- 独立したUI状態を持つ
- 複数箇所で使用する
- テスト対象として切り分けたい
- ファイルが長いだけでなく、変更理由が複数ある

行数だけを理由に分割せず、責務と変更理由で判断します。

## テスト方針

フロントエンドテストは、見た目の細部を固定するためではなく、重要な操作と状態を守るために使います。

優先して固定する対象:

- loading中の重複操作防止
- error、empty、selectedの表示
- タブ・モーダル・スワイプの主要操作
- URL生成、表示補助などのUtility
- propsに基づく重要な表示分岐
- モバイル固有の操作条件

単純な静的表示や、変更頻度の高い装飾だけを大量にsnapshot testへ固定しません。

## 変更時の確認項目

- Page、Feature Component、Common Componentの責務が分かれているか
- Componentへ業務判断やDB操作を入れていないか
- propsに不要な内部情報を渡していないか
- モバイル縦・横・タブレット・PCで表示を確認したか
- 横スクロール、固定要素の重なり、タップ競合がないか
- loading、error、empty、selected状態を確認したか
- Effectsが操作UIを妨げていないか
- 自動送りやアニメーションを停止できるか
- ラフ画像から仕様外UIを追加していないか
- 必要なVitestが追加・更新されているか
