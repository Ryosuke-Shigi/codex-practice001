...
各タブは独立したComponentとして作成する。
Page Componentは、タブ選択、固定データの受け渡し、全体レイアウトだけを担当する。

MOCK Componentには、DB保存、API通信、Storage保存、CSV生成、PDF生成、認証・権限判断、正式な業務状態遷移を入れない。
...
- 一覧・詳細・カード・表の見え方
- タブや画面導線
- loading / error / empty / selected などの画面状態
- モバイル表示
- 利用者が迷わないか
- PRODUCT化時に固定すべきUI契約

MOCKのコードをそのままPRODUCTへ昇格しない。
PRODUCT化する場合は、MOCKで確認した画面仕様・導線・状態を抽出し、改めてAction / Service / Repository / DTO / Responder / Testを設計する。
...
案件固有のUI契約は、必要に応じて `docs/features/` などの個別文書へ分離します。
