...
実装変更時は、関連コメントだけでなく、PHPDoc、JSDoc、型アノテーション、props契約説明、責務説明が現在の実装と一致しているか確認します。

責務、入力、出力、DTO形、状態遷移、例外条件、外部API境界、props契約が変わった場合は、対応する説明も更新します。古い説明を残す場合は、実装と説明のズレとして扱います。

目的は不要なコメント追加ではなく、実装と説明のズレを残さないことです。型やシグネチャから明らかな内容を重複して書かず、コメントやアノテーションで責務混在、仕様未確定、テスト不足を隠しません。

古いコメントや説明を見つけた場合は、現在の実装に合わせて修正するか、不要であれば削除します。判断に迷う場合は、責務境界や仕様を人間に確認してから更新します。
