...
指示用まとめは、ChatGPTが作成・圧縮・圧縮後チェックを行う一時的な作業条件です。作成・圧縮・圧縮後チェックはCodexApp側の作業ではありません。

毎回の指示用まとめ本文、会話ログ、PR固有の一時情報は恒久docsへ保存しません。

CodexAppは、ChatGPTから渡された指示用まとめに従って対象MD確認、既存コード調査、実装、差分確認、テスト実行、ユーザーの明示指示がある場合のPR作成を行います。CodexAppは指示用まとめを作成、再設計、再圧縮、省略しません。

CodexAppは、指示用まとめに含まれる目的、作業範囲、作るもの、作らないもの、読むMD、読まないMD、責務境界、成功条件、失敗条件、テスト観点、確認コマンド、PR前レビュー観点を削除・変更しません。

不明点や矛盾がある場合は、推測で補完せず停止して報告します。
